...
interface LangRow {
  id: number; slug: string; title: string; sort_order: number; is_published: boolean
}
...
export default function ProjectsList() {
  ...
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('projects').select('id, slug, title, sort_order, language_code, translation_group_id, is_published').order('sort_order')
    if (data) {
      const map = new Map<string, ProjectGroup>()
      for (const row of data as any[]) {
        if (!map.has(row.translation_group_id)) {
          map.set(row.translation_group_id, { translation_group_id: row.translation_group_id, en: null, es: null })
        }
        const g = map.get(row.translation_group_id)!
        g[row.language_code as 'en' | 'es'] = { id: row.id, slug: row.slug, title: row.title, sort_order: row.sort_order, is_published: row.is_published }
      }
  ...
  const LangCell = ({ lang, row }: { lang: string; row: LangRow | null }) => (
    <td className="px-4 py-3">
      <div className="flex items-center justify-center gap-2">
        {row ? (
          <>
            <button onClick={() => navigate(`/admin/projects/${row.slug}/edit`)} className={`inline-flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-full transition-all ${
              row.is_published
  ...
  return (
    <div>
            ...
            <tbody>
              {groups.map((g) => (
                <tr key={g.translation_group_id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                  <td className="px-4 py-3 text-white font-medium">{g.en?.title || g.es?.title || '—'}</td>
      ...
      <ConfirmDialog open={!!deleteId} title="Delete Project" message="This will delete only the selected language version." onConfirm={handleDelete} onCancel={() => setDeleteId(null)} loading={deleting} />
    </div>
  )
}
